...
@endif>
    ...
    <div class="admin_panel">
                    ...
                    <div class="card" style="width: 100%; background-color: transparent; border: 1px solid white; color: white; margin: 0 auto; border-radius: 5px; margin-top: 10px; position: relative;">
                        ...
                        @if(Auth::user())
                            @if(Auth::user()->admin || Auth::user()->id == $card->user_id)
                                <a href="{{ route("forum.delete", $card->id) }}"><button class="delete_btn">Delete</button></a>
                            @endif
                        @endif
                    </div>
                </a>
        ...
        @if(count($applys) > 0)
            @foreach($applys as $card)
                <div class="card" style="width: 100%; background-color: transparent; border: 1px solid white; color: white; margin: 0 auto; border-radius: 5px; margin-top: 10px; position: relative;">
                    ...
                    @if(Auth::user())
                        @if(Auth::user()->admin || Auth::user()->id == $card->user_id)
                            <a href="{{ route("staff_applications.delete", $card->id) }}"><button class="delete_btn">Delete</button></a>
                        @endif
                    @endif
                </div>
            @endforeach
        ...
        @endif
        <p class="title" style="cursor: pointer;">Messages</p>
        @if(count($messages) > 0)
            <div style="position: relative;">
                <a href="{{ route("chat.download") }}"><button style="padding: 10px 20px; background-color: rgb(87, 143, 87); color: white; border: none">Download Log</button></a>
                @if(Auth::check() && Auth::user()->admin)
                    <a href="{{ route("chat.delete_all") }}"><button class="delete_btn">Delete all Messages</button></a>
                @endif
            </div>
        @else
            <p style="text-align: center; color: white; font-family: Arial; margin-top: 10px;">No messages found</p>
        @endif   
    </div>
</body>
</html>